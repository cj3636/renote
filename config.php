...
function env_bool($key, $default=false){ $v = $_ENV[$key] ?? getenv($key); if ($v === null) return $default; $v = strtolower(trim($v)); return in_array($v, ['1','true','yes','on'], true); }
function env_int($key, $default){ $v = $_ENV[$key] ?? getenv($key); if ($v === null || $v==='') return $default; return (int)$v; }
function env_str($key, $default){ $v = $_ENV[$key] ?? getenv($key); return ($v===null||$v==='') ? $default : $v; }
